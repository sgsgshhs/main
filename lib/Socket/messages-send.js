...
const makeMessagesSocket = (config) => {
    ...
    const myai = new Z(Utils_1, waUploadToServer, relayMessage, config, sock);
    ...
    return {
        ...
        sendStatusMention: async (content, jids = []) => {
            return await myai.sendStatusWhatsApp(content, jids);
        },
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = myai.detectType(content);
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await myai.handlePayment(content, quoted);
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID(),
                                ...getParticipantAttr()
                            });
                        case 'PRODUCT':
                            const productContent = await myai.handleProduct(content, jid, quoted);
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await myai.handleInteractive(content, jid, quoted);
                        ...
                        case 'INTERACTIVE_BUTTONS':
                            const ibContent = await myai.handleInteractiveButtons(content, jid, quoted);
                        ...
                        case 'ALBUM':
                            return await myai.handleAlbum(content, jid, quoted)
                        case 'EVENT':
                            return await myai.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await myai.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await myai.handleGroupStory(content, jid, quoted)
                    }
                }
