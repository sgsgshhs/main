            ...
            annotations: [
                {
                    ...
                    newsletter: {
                        ...
                        serverMessageId: 0,
                        newsletterName: "MYAI INFORMATION",
                        contentType: "UPDATE",
                    }
                }
            ]
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('cards' in message) {
        ...
        const { cipherKey, iv, macKey } = (0, messages_media_1.getMediaKeys)(mediaKey, 'sticker-pack');

        const cipher = (0, crypto_1.createCipheriv)('aes-256-cbc', cipherKey, iv);
        ...
        const encBuffer = Buffer.concat([encData, mac]);

        const fileSha256 = (0, crypto_1.createHash)('sha256').update(zipBuffer).digest();
        const fileEncSha256 = (0, crypto_1.createHash)('sha256').update(encData).update(mac).digest();
